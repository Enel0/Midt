// Regiones y comunas de Chile (resumen completo)
export const REGIONES_COMUNAS = [
  ...
  { region: "Valparaíso", comunas: ["Valparaíso", "Viña del Mar", "Concón", "Quintero", "Puchuncaví", "Quilpué", "Villa Alemana", "Limache", "Olmué", "Quillota", "La Calera", "Hijuelas", "La Cruz", "Nogales", "San Antonio", "Cartagena", "El Quisco", "El Tabo", "Algarrobo", "Casablanca", "Isla de Pascua", "San Felipe", "Llaillay", "Catemu", "Panquehue", "Putaendo", "Santa María", "Los Andes", "Calle Larga", "Rinconada", "San Esteban"] },
  ...
  { region: "Maule", comunas: ["Talca", "Constución", "Curepto", "Empedrado", "Maule", "Pelarco", "Pencahue", "Río Claro", "San Clemente", "San Rafael", "Cauquenes", "Chanco", "Pelluhue", "Curicó", "Hualañé", "Licantén", "Molina", "Rauco", "Romeral", "Sagrada Familia", "Teno", "Vichuquén", "Linares", "Colbún", "Longaví", "Parral", "Retiro", "San Javier", "Villa Alegre", "Yerbas Buenas"] },
...
export const comunasByRegion = (region) => REGIONES_COMUNAS.find((r) => r.region === region)?.comunas || [];

// Validación de RUT (formato 12.345.678-5 y dígito verificador correcto)
...
export function validarRutDV(rut) {
  const clean = rut.replace(/\./g, '').replace(/-/g, '').toUpperCase();
  const body = clean.slice(0, -1);
  const dv = clean.slice(-1);
  if (!/^[0-9]+$/.test(body)) return false;
  const expected = calcularDV(parseInt(body, 10));
  return expected === dv;
}

export function formatearRut(rut) {
  const clean = rut.replace(/\D/g, '').toUpperCase();
  if (clean.length < 2) return rut;
  const body = clean.slice(0, -1);
  const dv = rut.replace(/\D/g, '').slice(-1) || clean.slice(-1);
